...
def show_simple_card(icon, title, description, search_keyword):
    """画像を使わず、アイコンとテキストで見せるシンプルなカード"""
    # 枠線付きのカード
...
def show_affiliate_links(category_name):
    """カテゴリに応じたおすすめ商品を表示する (テキスト＆アイコン版)"""
    st.divider()
    st.subheader("🛒 記録向上のための厳選アイテム")
    st.caption(f"※{category_name}選手におすすめのギア")
    ...
    # --- 全員におすすめ (下段) ---
    st.markdown("") 
    st.markdown("##### 🥤 全アスリートにおすすめ")
    c1, c2, c3 = st.columns(3)
    
    with c1:
        show_simple_card("🥛", "プロテイン", "体づくりに", "ザバス プロテイン")
    with c2:
        show_simple_card("🌀", "ケア用品", "筋膜リリース", "フォームローラー")
    with c3:
        show_simple_card("⚡", "アミノ酸", "BCAAなど", "アミノバイタル")
