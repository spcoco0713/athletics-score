...
TEXT_RES = {
    "日本語": {
        ...
        "error_no_file": "現在、男子のデータファイル (CSV) が見つかりません。",
    ...
    "English": {
        ...
        "error_no_file": "Data file (CSV) for Men not found.",
...
if df is not None:
    ...
    for cat in categorized_events:
        ...
        sorted_priority = sorted(priority_items, key=lambda x: top_keys.index(all_events_map[x]) if all_events_map[x] in top_keys else 999)
    ...
    st.info(get_text("error_no_file", lang_choice))
